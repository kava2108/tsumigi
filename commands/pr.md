...
argument-hint: "<issue-id> [--draft] [--base <branch>] [--post-checklist]"
---
...
- $ARGUMENTS がない場合は「引数に issue-id を指定してください（例: /tsumigi:pr GH-123）」と言って終了する
- $ARGUMENTS を解析する：
  ...
  - issue_id が `GH-NNN` 形式の場合、NNN を github_issue_number に設定
...
- `docs/drift/{{issue_id}}/drift-report.md` が存在する場合 Read し、drift_score を取得する（存在しない場合は "未計測"）
- `docs/sync/{{issue_id}}/sync-report.md` が存在する場合 Read し、consistency_score を取得する（存在しない場合は "未計測"）
- `docs/implements/{{issue_id}}/` 以下から最後に使われた impl_mode を確認する（tdd/direct/不明）
- step4 を実行する
...
post_checklist が true の場合、または `docs/reviews/{{issue_id}}/review-checklist.md` が存在する場合は実行する：

- `docs/reviews/{{issue_id}}/review-checklist.md` が存在するか確認する
  - 存在しない場合：「レビューチェックリストが未生成です。`/tsumigi:review {{issue_id}}` を先に実行することを推奨します」と案内してスキップする
  - 存在する場合：ファイルを Read する
- Bash で PR にコメントとして投稿する：
  ```bash
  gh pr comment {{pr_number}} --body "$(cat docs/reviews/{{issue_id}}/review-checklist.md)" 2>&1
  ```
