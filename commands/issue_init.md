...
argument-hint: "<issue-id> [issue-url-or-text] [--scope full|lite] [--issue <number>]"
---
...
- $ARGUMENTS がない場合は「引数に issue-id を指定してください（例: /tsumigi:issue_init 001-feature-name）」と言って終了する
- $ARGUMENTS を解析する：
  - `--scope full` または `--scope lite` を確認し context に設定（デフォルト: full）
  - `--issue` の後の数値を github_issue_number に設定
  - 最初のトークンを issue_id に設定
  - 残りのテキスト（`--` フラグを除く）を issue_source に設定
...
- Bash で現在のブランチを確認する：
  ...
  ```
- ブランチ名に issue_id が含まれない場合（例: main, develop）：
  - AskUserQuestion ツールで確認する：
    - question: "現在のブランチ（<branch名>）で作業を続けますか？ Issue 専用ブランチ（例: feature/{{issue_id}}）の使用を推奨します。"
    - options: ["このブランチで続ける", "中断する（手動でブランチを切る）"]
  - 「中断する」が選ばれた場合：
    - 以下を案内して終了する：
      ```
...
**GitHub Issue の場合**（github_issue_number が設定されている、または issue_source に `github.com/*/issues/` を含む）:

- issue 番号を特定する：
  - github_issue_number が設定されている場合はその値を使用
  - issue_source が GitHub URL の場合は URL 末尾の数字を使用
...
- 既存 IMP ファイルを Glob で確認する：`specs/{{issue_id}}/IMP.md`
  - 依存関係にある Issue の IMP があれば内容を把握する

## step7: 構造化 Issue 定義の生成
